...
export function ProductsList() {
    ...
    for (let i = 0; i < Math.floor(Math.random() * 30); i++) {
        productsCount.push(<ProductsDiv key={i} />);
    }
    for (let i = 0; i < Math.floor(Math.random() * 60); i++) {
        if (i % 5 === 0) {
            filterCount.push(
                <div key={i} className={classes.filter_container}>
                    <input type={"checkbox"} />
                ...
                <div className={classes.productsList_container}>
                    <div className={classes.productsList_info}>
                        <label className={classes.filter_ico} htmlFor={"filter__toggle"}>
                            <img
                                className={classes.open_filter_img}
                                src="317-3175538_png-file-filter-list-icon-transparent-png.png"
                            ></img>
                            <img
                                className={classes.close_filter_img}
                                src="image_processing20200511-32371-1owpt6i.png"
                            ></img>
                        </label>
                        <h3 className={classes.h3}>
                            Categories: GenshinImpact {productsCount.length}
                        </h3>
                        <select className={classes.sort_select} name="Sort" id="sort-select">
                            <option className={classes.select_option} value="Expensive">
                                Expensive
                            </option>
                            <option className={classes.select_option} value="Сheap">
                                Сheap
                            </option>
                            <option className={classes.select_option} value="Low reviews">
                                Low reviews
                            </option>
                            <option className={classes.select_option} selected value="High reviews">
                                High reviews
                            </option>
                        </select>
                    </div>
                    <div className={classes.product_list}>{productsCount}</div>
